...
for i = 1:N
    ...
    for j=1:length(ww)
        ...
        y = yy(j);
        z = zz(j);
        m = [x^3*w,w*x^2*y,y^2*w*x,y^3*w,w*x^2*z,w*x*y*z,z*y^2*w,z^2*w*x,z^2*y*w,z^3*w,x^2*w,x^3,x*y*w,x^2*y,y^2*w,y^2*x,y^3,x*z*w,x^2*z,z*y*w,y*x*z,z*y^2,z^2*w,x*z^2,z^2*y,z^3,x*w,x^2,w*y,y*x,y^2,w*z,x*z,z*y,z^2,w,x,y,z,1];
        m = m/norm(m,'fro');
        M = [M; norm(C*m.','fro')];
    end
    M = sort(M);
