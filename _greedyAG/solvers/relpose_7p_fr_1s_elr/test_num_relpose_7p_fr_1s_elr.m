...
for i = 1:N
    ...
    try
        ...
        [xx,yy,zz] = nstd_22x41_colpiv_relpose_7p_fr_1s_elr(C); % solve polynomial system
        tm = toc;
    ...
    for j=1:length(xx)
        ...
        y = yy(j);
        z = zz(j);
        m = [x^2*y^2*z^2,x*y^3*z^2,y^4*z^2,x^3*y*z,x^2*y^2*z,x*y^3*z,y^4*z,x^2*y*z^2,x*y^2*z^2,y^3*z^2,x^4,y*x^3,y^2*x^2,y^3*x,y^4,x^3*z,x^2*y*z,x*y^2*z,z*y^3,x^2*z^2,x*y*z^2,y^2*z^2,x^3,y*x^2,x*y^2,y^3,x^2*z,y*z*x,y^2*z,x*z^2,y*z^2,x^2,x*y,y^2,z*x,y*z,z^2,x,y,z,1];
        m = m/norm(m,'fro');
        M = [M; norm(C*m.','fro')];
    end
    M = sort(M);
...
fprintf('Problem: relpose_7p_fr_1s_elr. Ave. runtime: %0.1f ms. Med. error: %0.2e\n',10^3*mean(Tm_relpose_7p_fr_1s_elr),median(Err_relpose_7p_fr_1s_elr));
