...
for i = 1:N
    ...
    for j=1:length(xx)
        ...
        y = yy(j);
        z = zz(j);
        m = [x^3,x^2*y,x*y^2,y^3,z*x^2,z*x*y,y^2*z,z^2*x,z^2*y,z^3,x^2,x*y,y^2,z*x,z*y,z^2,x,y,z,1];
        m = m/norm(m,'fro');
        M = [M; norm(C*m.','fro')];
    end
    M = sort(M);
...
fprintf('Problem: satellite_triang. Ave. runtime: %0.1f ms. Med. error: %0.2e\n',10^3*mean(Tm_satellite_triang),median(Err_satellite_triang));
