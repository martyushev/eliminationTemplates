...
for i = 1:N
    ...
    for j=1:length(gg)
        ...
        u = uu(j);
        v = vv(j);
        m = [g*u^3,g*u^2*v,g*u*v^2,g*v^3,u^2*g,u^3,v*u*g,v*u^2,v^2*g,v^2*u,v^3,g*u,u^2,v*g,v*u,v^2,g,u,v,1];
        m = m/norm(m,'fro');
        M = [M; norm(C*m.','fro')];
    end
    M = sort(M);
    err = norm(M(1:min(9,length(M))),'fro');
...
fprintf('Problem: focal6p_1s. Ave. runtime: %0.1f ms. Med. error: %0.2e\n',10^3*mean(Tm_focal6p_1s),median(Err_focal6p_1s));
