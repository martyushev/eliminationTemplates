function coefs_optpose4pt_v2(data::Vector{Matrix{Float64}})

    U = data[1]
    u = data[2]

    C = zeros(5,19)
    C[95] = -1
    C[[45,70]] .= 1
    C[91] = u[1]*U[1]^2
    C[92] = u[5]*U[4]^2
    C[93] = u[9]*U[7]^2
    C[94] = u[13]*U[10]^2
    C[71] = 2*u[1]*U[1]
    C[76] = 2*u[2]*U[1]
    C[46] = 2*u[2]*U[2]
    C[26] = 2*u[2]*U[3]
    C[72] = 2*u[5]*U[4]
    C[77] = 2*u[6]*U[4]
    C[47] = 2*u[6]*U[5]
    C[27] = 2*u[6]*U[6]
    C[73] = 2*u[9]*U[7]
    C[78] = 2*u[10]*U[7]
    C[48] = 2*u[10]*U[8]
    C[28] = 2*u[10]*U[9]
    C[74] = 2*u[13]*U[10]
    C[79] = 2*u[14]*U[10]
    C[49] = 2*u[14]*U[11]
    C[29] = 2*u[14]*U[12]
    C[86] = 2*u[2]*U[2]*U[1]
    C[81] = 2*u[2]*U[3]*U[1]
    C[87] = 2*u[6]*U[5]*U[4]
    C[82] = 2*u[6]*U[6]*U[4]
    C[88] = 2*u[10]*U[8]*U[7]
    C[83] = 2*u[10]*U[9]*U[7]
    C[89] = 2*u[14]*U[11]*U[10]
    C[84] = 2*u[14]*U[12]*U[10]
    C[11] = 2*u[2]
    C[16] = 2*u[3]
    C[12] = 2*u[6]
    C[17] = 2*u[7]
    C[13] = 2*u[10]
    C[18] = 2*u[11]
    C[14] = 2*u[14]
    C[19] = 2*u[15]
    C[31] = 2*u[1]*U[2]+2*u[3]*U[3]
    C[51] = -2*u[1]*U[3]+2*u[3]*U[2]
    C[56] = 2*u[4]*U[2]-2*u[3]*U[3]
    C[36] = 2*u[4]*U[3]+2*u[3]*U[2]
    C[32] = 2*u[5]*U[5]+2*u[7]*U[6]
    C[52] = -2*u[5]*U[6]+2*u[7]*U[5]
    C[57] = 2*u[8]*U[5]-2*u[7]*U[6]
    C[37] = 2*u[8]*U[6]+2*u[7]*U[5]
    C[33] = 2*u[9]*U[8]+2*u[11]*U[9]
    C[53] = -2*u[9]*U[9]+2*u[11]*U[8]
    C[38] = 2*u[11]*U[8]+2*u[12]*U[9]
    C[58] = -2*u[11]*U[9]+2*u[12]*U[8]
    C[34] = 2*u[13]*U[11]+2*u[15]*U[12]
    C[54] = -2*u[13]*U[12]+2*u[15]*U[11]
    C[39] = 2*u[15]*U[11]+2*u[16]*U[12]
    C[59] = -2*u[15]*U[12]+2*u[16]*U[11]
    C[41] = u[1]*U[2]^2+u[4]*U[3]^2+2*u[3]*U[2]*U[3]
    C[66] = u[1]*U[3]^2+u[4]*U[2]^2-2*u[3]*U[2]*U[3]
    C[42] = u[5]*U[5]^2+u[8]*U[6]^2+2*u[7]*U[5]*U[6]
    C[67] = u[5]*U[6]^2+u[8]*U[5]^2-2*u[7]*U[5]*U[6]
    C[44] = u[13]*U[11]^2+u[16]*U[12]^2+2*u[15]*U[11]*U[12]
    C[68] = -2*u[11]*U[8]*U[9]+u[9]*U[9]^2+u[12]*U[8]^2
    C[43] = 2*u[11]*U[8]*U[9]+u[9]*U[8]^2+u[12]*U[9]^2
    C[69] = -2*u[15]*U[11]*U[12]+u[13]*U[12]^2+u[16]*U[11]^2
    C[61] = 2*u[3]*U[2]^2-2*u[3]*U[3]^2-2*u[1]*U[2]*U[3]+2*u[4]*U[2]*U[3]
    C[62] = 2*u[7]*U[5]^2-2*u[7]*U[6]^2-2*u[5]*U[5]*U[6]+2*u[8]*U[5]*U[6]
    C[64] = 2*u[15]*U[11]^2-2*u[15]*U[12]^2-2*u[13]*U[11]*U[12]+2*u[16]*U[11]*U[12]
    C[63] = -2*u[9]*U[8]*U[9]+2*u[12]*U[8]*U[9]+2*u[11]*U[8]^2-2*u[11]*U[9]^2
    C[[1,6]] .= u[1]
    C[21] = u[4]
    C[[2,7]] .= u[5]
    C[22] = u[8]
    C[[3,8]] .= u[9]
    C[23] = u[12]
    C[[4,9]] .= u[13]
    C[24] = u[16]

    C = C./repeat(sqrt.(sum(C.*conj(C),dims=2)),1,size(C,2))

    U(v,w,x,y,z) = [v^2,w^2,x*v,w*x,x^2,v*y,w*y,x*y,y^2,v*z,w*z,x*z,y*z,z^2,v,x,y,z,1]

    return C,U

end