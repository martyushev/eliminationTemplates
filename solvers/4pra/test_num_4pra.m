...
for i = 1:N
    ...
    for j=1:length(uu)
        ...
        v = vv(j);
        w = ww(j);
        m = [u^4,u^3*v,u^2*v^2,v^3*u,v^4,w*u^3,u^2*v*w,w*v^2*u,w*v^3,u^2*w^2,w^2*v*u,v^2*w^2,u*w^3,v*w^3,w^4,u^3,u^2*v,u*v^2,v^3,u^2*w,u*v*w,w*v^2,u*w^2,v*w^2,w^3,u^2,u*v,v^2,u*w,v*w,w^2,u,v,w,1];
        m = m/norm(m,'fro');
        M = [M; norm(C*m.','fro')];
    end
    M = sort(M);
