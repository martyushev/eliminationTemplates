...
for i = 1:N
    ...
    try
        ...
        [aa,bb,pp] = nstd_40x46_colpiv_relpose_7p_fuv_angle(C); % solve polynomial system
        tm = toc;
    ...
    M = sort(M);
    err = norm(M(1:min(6,length(M))),'fro');
...
fprintf('Problem: relpose_7p_fuv_angle. Ave. runtime: %0.1f ms. Med. error: %0.2e\n',10^3*mean(Tm_relpose_7p_fuv_angle),median(Err_relpose_7p_fuv_angle));
