...
for i = 1:N
    ...
    for j=1:length(x1x1)
        ...
        x4 = x4x4(j);
        x5 = x5x5(j);
        m = [x1^2,x1*x2,x2^2,x3*x1,x3*x2,x3^2,x4*x1,x4*x2,x4*x3,x4^2,x5*x1,x5*x2,x5*x3,x4*x5,x5^2,x1,x2,x3,x4,x5,1];
        m = m/norm(m,'fro');
        M = [M; norm(C*m.','fro')];
    end
    M = sort(M);
...
fprintf('Problem: gp4p_scale. Ave. runtime: %0.1f ms. Med. error: %0.2e\n',10^3*mean(Tm_gp4p_scale),median(Err_gp4p_scale));
