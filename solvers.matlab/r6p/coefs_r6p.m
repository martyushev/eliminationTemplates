function [C,U,dU] = coefs_r6p(data)
    ...
    C(25) = -q(1)*A(29)+q(2)*A(27)+q(2)*q(1)*A(30)-q(2)*X(3)*q(1)-A(26);
    C(32) = q(1)*A(34)-X(2)*q(1)^2-q(1)^2*A(36)-q(1)*A(33)+A(31);
    C(31) = -q(1)*A(35)+q(2)*A(33)+q(2)*q(1)*A(36)+q(2)*X(2)*q(1)-A(32);
    ...
    C(79) = -q(2)*X(1)*q(1)+q(2)*q(1)*A(84)-A(80)-q(1)*A(83)+q(2)*A(81);
    C(61) = q(2)*q(1)*A(66)-A(62)+q(2)*A(63)-q(2)*X(1)-q(1)*A(65);
    C(67) = q(2)*q(1)*A(72)-A(68)-X(1)+q(2)*A(69)-q(1)*A(71);
    C(85) = q(2)*q(1)*A(90)-A(86)-q(1)*A(89)+q(2)*A(87)-X(1)*q(1);
    ...
    C(58) = A(55)-X(5)*q(3)-q(3)^2*A(60)+q(3)*A(58)-q(3)*A(57);
    ...
    C(70) = A(67)-X(5)-q(3)^2*A(72)+q(3)*A(70)-q(3)*A(69);
    C(72) = A(67)-X(8)+q(5)*A(70)-q(5)*A(69)-q(5)^2*A(72);
    C(76) = A(73)-X(5)*q(3)^2-q(3)^2*A(78)+q(3)*A(76)-q(3)*A(75);
    ...
    C(86) = A(85)+q(1)*A(88)-q(1)*A(87)-q(1)^2*A(90)-X(2)*q(1);
    C(88) = A(85)-q(3)*A(87)-q(3)^2*A(90)-X(5)*q(3)+q(3)*A(88);
    ...
    C(91) = q(2)*A(93)+q(2)*X(3)-q(1)*A(95)+q(2)*q(1)*A(96)-A(92)-X(2);
    C(94) = q(3)*A(94)-q(3)^2*A(96)-q(3)*A(93)-q(3)*X(6)+A(91)+X(4);
    C(93) = -q(3)*A(95)+q(4)*A(93)+q(4)*X(6)+q(4)*q(3)*A(96)-A(92)-X(5);
    C(96) = q(5)*A(94)-q(5)^2*A(96)-q(5)*A(93)-q(5)*X(9)+A(91)+X(7);
    C(95) = q(6)*A(93)+q(6)*X(9)-q(5)*A(95)+q(6)*q(5)*A(96)-A(92)-X(8);
    C(73) = q(2)*X(2)*q(1)+q(2)*q(1)*A(78)-A(74)-q(1)*A(77)+q(2)*A(75)+q(1)*X(3);
    ...
    C(64) = A(61)+X(6)+X(4)*q(3)-q(3)^2*A(66)+q(3)*A(64)-q(3)*A(63);
    C(66) = A(61)+X(9)+q(5)*A(64)-q(5)*A(63)+X(7)*q(5)-q(5)^2*A(66);
    C(80) = A(79)+q(1)*A(82)-q(1)*A(81)+X(1)*q(1)^2-q(1)^2*A(84)+q(1)*X(3);
    C(82) = A(79)-q(3)*A(81)+X(4)*q(3)^2-q(3)^2*A(84)+q(3)*X(6)+q(3)*A(82);
    C(84) = A(79)+q(5)*X(9)+q(5)*A(82)-q(5)*A(81)+X(7)*q(5)^2-q(5)^2*A(84);

    C = C./repmat(sqrt(sum(C.*conj(C),2)),1,size(C,2));

    U = @(S,n)[S(4,:).*S(1,:);S(4,:).*S(2,:);S(4,:).*S(3,:);S(5,:).*S(1,:);S(5,:).*S(2,:);S(5,:).*S(3,:);S(1,:).*S(6,:);S(6,:).*S(2,:);S(6,:).*S(3,:);S(1,:);S(2,:);S(3,:);S(4,:);S(5,:);S(6,:);ones(1,n)];
    dU = @(S,n)[[S(4,:);zeros(1,n);zeros(1,n);S(5,:);zeros(1,n);zeros(1,n);S(6,:);zeros(1,n);zeros(1,n);ones(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n)],[zeros(1,n);S(4,:);zeros(1,n);zeros(1,n);S(5,:);zeros(1,n);zeros(1,n);S(6,:);zeros(1,n);zeros(1,n);ones(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n)],[zeros(1,n);zeros(1,n);S(4,:);zeros(1,n);zeros(1,n);S(5,:);zeros(1,n);zeros(1,n);S(6,:);zeros(1,n);zeros(1,n);ones(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n)],[S(1,:);S(2,:);S(3,:);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);ones(1,n);zeros(1,n);zeros(1,n);zeros(1,n)],[zeros(1,n);zeros(1,n);zeros(1,n);S(1,:);S(2,:);S(3,:);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);ones(1,n);zeros(1,n);zeros(1,n)],[zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);S(1,:);S(2,:);S(3,:);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);zeros(1,n);ones(1,n);zeros(1,n)]];
